...
Windows（可选）：双击 [`install-latest.cmd`](./install-latest.cmd) 会把 `main` fast-forward 到 `origin/main`，并静默安装一份未签名的并排 **grok-app-latest**（不覆盖正式版 **Grok**）。需要 VS Build Tools + Rust MSVC；详见 [docs/BUILD.md](./docs/BUILD.md)。CI 还会在 `main` 更新时发布滚动 **nightly** 预发布（配置了 Apple / Windows secrets 时会签名）；这不是 GitHub 的 “latest”。
